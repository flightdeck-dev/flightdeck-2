...
**C1: `running→done` bypasses verification (state machine + facade)**
...
**C2: Side effects are computed but discarded**
...
**M1: MCP server has no caller identity / auth (Layer 3 missing)**
...
**M2: `flightdeck_task_submit` ignores the `claim` parameter**
...
**M3: `flightdeck_discuss` is a no-op**
...
**M4: Orchestrator `ended` session handling resets to `ready` without state machine validation**
...
**M5: `MessageLog.channels()` uses `require()` inside an ESM module**
...
**M6: Governance `shouldGateTaskStart` checks task state, not profile rules**
...
**m3: `SpecStore.write()` doesn't ensure directory exists**
...
**m4: `topoSort()` doesn't detect cycles**
...
**m5: CLI `start` command is a stub that doesn't actually start the orchestrator**
...
**S5: Add `flightdeck_task_add` MCP tool**
