...
describe('Governance shouldGateTaskStart', () => {
  it('autonomous mode never gates', () => {
    const gov = new GovernanceEngine({ name: 'test', governance: 'autonomous', isolation: 'file_lock', onCompletion: 'stop' });
  ...
  it('collaborative mode always gates', () => {
    const gov = new GovernanceEngine({ name: 'test', governance: 'collaborative', isolation: 'file_lock', onCompletion: 'stop' });
  ...
  it('supervised mode gates workers but not reviewers', () => {
    const gov = new GovernanceEngine({ name: 'test', governance: 'supervised', isolation: 'file_lock', onCompletion: 'stop' });
    expect(gov.shouldGateTaskStart('ready', 'worker')).toBe(true);
    expect(gov.shouldGateTaskStart('ready', 'reviewer')).toBe(false);
  });
});
