...
import { Folder, FolderOpen, LayoutDashboard, MessageSquare, ListTodo, Bot, Scale, Settings, ChevronDown, ChevronRight, PanelLeftClose, PanelLeft, Plus, MoreHorizontal, Trash2, Archive, X, Clock, Crown, AlertTriangle, FileText, Activity, Radio } from 'lucide-react';
      ...
      {!collapsed && (activeAgents > 0 || hibernatedAgents > 0) && (
        <div className="px-3 py-2 text-xs text-[var(--color-text-secondary)]">
          <div className="flex items-center gap-2">
            {activeAgents > 0 && (<>
              <span className={`w-2 h-2 rounded-full ${busyAgents > 0 ? 'bg-[var(--color-status-running)] animate-pulse' : 'bg-[var(--color-text-tertiary)]'}`} />
