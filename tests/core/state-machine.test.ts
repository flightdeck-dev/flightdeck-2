...
describe('State Machine', () => {
  ...
  it('emits spawn_reviewer effect on running->done', () => {
    const result = transition('running', 'done', { taskId: 'task-abc' as TaskId });
    expect(result.effects).toHaveLength(2); // spawn_reviewer + update_dag
